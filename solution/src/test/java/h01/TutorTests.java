...
class TutorTests {
	...
	private static final CharSequence BISHOP_WIN = "Der L\u00E4ufer hat gewonnen!";
	private static final CharSequence ROOK_WIN = "Der Turm hat gewonnen!";
	...
	static void initTest() {
		...
		Task1.setNextFrameDelay(0);
		Task1.setUiVisible(false);
		columns.addAll(fours);
		rows.addAll(fours);
		traces = new ArrayList<>();
		for (var i = 0; i < RUNS; i++) {
			try {
				// With JDK 9 and 12 some minor changes where implemented that change how such a hack can work. Tested with JDK 14.
				setFinalStatic(Task1.class.getDeclaredField("NUMBER_OF_ROWS"), rows.get(i));
				setFinalStatic(Task1.class.getDeclaredField("NUMBER_OF_COLUMNS"), columns.get(i));
			} catch (SecurityException e) {
				throw new RuntimeException("These test can not run in your current SecurityManager.", e);
			} catch (Exception e) {
				throw new RuntimeException("Properties could not be changed in a reflective way.", e);
			}
			try {
				Task1.main(null);
			} catch (Exception e) {
	...
	private void flagFailure(boolean isRookTest) {
		if (flagErroneousRun) {
			fail("The student's code failed in a run.");
		}
	...
	public void H3_3_T3() {
		...
		System.setOut(originalOut);
		var results = Arrays.stream(out.split("\n")).filter(s -> LevenshteinDistance.computeLevenshteinDistance(ROOK_WIN, s) < 2
				|| LevenshteinDistance.computeLevenshteinDistance(BISHOP_WIN, s) < 2).collect(Collectors.toList());
		...
		for (int i = 0; i < rookLast.size(); i++) {
			...
			assertTrue(LevenshteinDistance.computeLevenshteinDistance(expectedLog, results.get(i)) < 2,
					String.format("Output Message [%s] did not match the expected output [%s]", results.get(i), expectedLog));
		}
	}
